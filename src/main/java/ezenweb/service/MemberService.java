...
public class MemberService implements UserDetailsService , OAuth2UserService<OAuth2UserRequest , OAuth2User> {
    ...
    public OAuth2User loadUser( OAuth2UserRequest userRequest) throws OAuth2AuthenticationException {
        ...
        if( !optional.isPresent() ){
            memberRepository.save( oauthDto.toentitiy() );  // entity 저장
        }
...
//        System.out.println(" 검증번호 : " + authkey + " 회원아이디 : " + mid  );
    ...
    //6. 아이디 찾기  [ 이름과 임에ㅣㄹ이 동일한 경우 프론트엔드에 표시 ]
    public String idfind(String mname, String memail){
        String idfind=null;
        Optional<MemberEntity> optional= memberRepository.findbyid(mname,memail);
        if(optional.isPresent()){
            idfind=optional.get().getMid();
        }
        return idfind;
    }
    //7. 패스워드 찾기 [  아이디와 이메일이 동일한 경우 이메일로 임시 비밀번호 (난수) 전송
    @Transactional
    public boolean pwdfind(String mid, String memail){
        ...
        if(optional.isPresent()){ //해당 엔티티를 찾았으면
            ...
            //2.비밀번호 임시비밀번호로 변경 ( 암호화 )
                //비크립트 암호화
            BCryptPasswordEncoder  passwordEncoder = new BCryptPasswordEncoder();
            //암호화 된 비밀번호
            optional.get().setMpassword(passwordEncoder.encode(tempassword));
            //3.변경된 비밀번호 이메일전송

            // 메일전송 구현
            StringBuilder html=new StringBuilder();
            html.append("<html><body>");
            html.append("<div>회원님의 임시비밀번호</div>");
            html.append("<div>"+tempassword+"로 입력하여주십시오.</div>");
            html.append("</body></html>");
            //메일  전송 메소드 호출
            mailsend(optional.get().getMemail(), "임시비밀번호",html);
             return true;
        }
        // 해당 엔티티를 못찾았으면
        return false;
